			...
			switch(vgmCommand)
			{
				...
				case 0x90:
					...
					if (TargetHardware.IsUsePCM)
					{
						...
						{	//	ストリーム ID が、すでに存在する場合は、同一ストリームID でセットアップが開始された警告表示をして、処理しない
				...
				case 0x91:
					...
					if (TargetHardware.IsUsePCM)
					{
						...
						var streamId = (int)tmp_d0;
						if (!m_vgmStreamDict.ContainsKey(streamId))
						{	//	ストリーム ID が、存在しない場合は、ストリームID が存在しない警告表示をして、処理しない						
						...
						AddVgmStreamDataToPlayImData(
							parseChip.ChipSelect, 
							a1				:	0, 
							code			:	(byte)0x91,
							vstrmStepSize	:	(byte)tmp_d0, 
							vstrmStepBase	:	(byte)tmp_d1
						);
					}
					break;
				...
				case 0x92:
					...
					if (TargetHardware.IsUsePCM)
					{
						...
						//	サンプリングレート	4Byte	読み込み
						if (!GetSourceData(strmSourceAddress+1, DataSize.DL, false, out tmp_d0)) return false;
						m_m6258SamplingRate = (int)tmp_d0;

						//	PLAY中間データに、サンプリングレートをVgmStreamSamplingRataで格納
						var parseChip = m_vgmStreamDict[streamId].m_parseChip;
						AddVgmStreamSamplingRataToPlayImData(
							parseChip.ChipSelect,
							a1				:	0,
							code			:	(byte)0x92, 
							vstrmSamplingRate:	m_m6258SamplingRate
						);
					}
					break;
				...
				case 0x93:
					...
					if (TargetHardware.IsUsePCM)
					{
						...
						//	データバンク内のデータ開始オフセット	4Byte 読み込み
						if (!GetSourceData(strmSourceAddress+1, DataSize.DL, false, out tmp_d0)) return false;
						//	長さモード（データ長の計算方法）		1Byte 読み込み
						...
						var parseChip = m_vgmStreamDict[streamId].m_parseChip;
						AddVgmStreamStartSizeToPlayImData(
							parseChip.ChipSelect, 
							a1				:	0, 
							code			:	(byte)0x93, 
							vstrmFlagMode	:	(byte)tmp_d1, 
							vstrmStart		:	(int)tmp_d0, 
							vstrmSize		:	(int)tmp_d2
						);
					}
					break;
				...
				case 0x95:
					...
					if (TargetHardware.IsUsePCM)
					{
						...
						AddVgmStreamStartSizeFastCallToPlayImData(
							parseChip.ChipSelect, 
							a1				:	0, 
							code			:	(byte)0x95, 
							vstrmBlockId	:	pcmBlockId, 
							vstrmFlagMode	:	flagMode,
							vstrmStart		:	pcmImData.m_pcmStart,
							vstrmSize		:	pcmImData.m_pcmSize,
							vstrmSamplingRate:	samplingRate
						);
					}
					break;
			}
